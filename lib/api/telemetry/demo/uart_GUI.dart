...
class _UartDisplayState extends State<UartDisplay> with SingleTickerProviderStateMixin {
  ...
  Map<String, double> _parameters = {}; // Store parameters as key-value pairs
  int _systemId = 1; // Default system ID for target system
  int _componentId = 0; // Default component ID for target system (0 = all components)
  ...
  void _setupMavlinkListener() {
    _parserSubscription = _parser.stream.listen((MavlinkFrame frm) {
      ...
      final String timeStr = "${timestamp.hour.toString().padLeft(2, '0')}:${timestamp.minute.toString().padLeft(2, '0')}:${timestamp.second.toString().padLeft(2, '0')}.${(timestamp.millisecond ~/ 10).toString().padLeft(2, '0')}";
      
      // Cập nhật dữ liệu log
      setState(() {
        _receivedData.add('[$timeStr] [MAVLink] Type: ${frm.message.runtimeType}, Sys: ${frm.systemId}, Comp: ${frm.componentId}');
        
        // Thêm một bản tóm tắt dữ liệu khi nhận được các gói tin quan trọng
        if (frm.message is Attitude || frm.message is GlobalPositionInt || 
            frm.message is VfrHud || frm.message is GpsRawInt || frm.message is SysStatus) {
          _addFlightSummary();
        }
        ...
        if (frm.message is Heartbeat) {
          ...
          _receivedData.add('  ├─ [Heartbeat] Type: ${_getSystemType(heartbeat.type)}, ' +
              'Autopilot: ${_getAutopilotType(heartbeat.autopilot)}, ' +
              'Base Mode: 0x${heartbeat.baseMode.toRadixString(16).padLeft(2, '0')}, ' +
              'Custom Mode: ${heartbeat.customMode}, ' +
              'System Status: ${_getSystemStatus(heartbeat.systemStatus)}');
          _receivedData.add('  └─ Version: MAVLink ${heartbeat.mavlinkVersion}');
        }
          ...
          _receivedData.add('  ├─ [SysStatus] Battery: ${_batteryPercent}%, ' +
              'Voltage: ${status.voltageBattery / 1000} V, ' +
              'Current: ${status.currentBattery / 100} A');
          _receivedData.add('  └─ CPU Load: ${status.load / 10}%, ' +
              'Comms Drop: ${status.dropRateComm}%, ' +
              'Errors: Comm: ${status.errorsComm}, ' +
          ...
          _receivedData.add('  ├─ [Attitude] Roll: ${_roll.toStringAsFixed(2)}°, ' +
              'Pitch: ${_pitch.toStringAsFixed(2)}°, ' +
              'Yaw: ${_yaw.toStringAsFixed(2)}°');
          _receivedData.add('  └─ Roll Rate: ${(attitude.rollspeed * 180 / pi).toStringAsFixed(2)}°/s, ' +
              'Pitch Rate: ${(attitude.pitchspeed * 180 / pi).toStringAsFixed(2)}°/s, ' +
              'Yaw Rate: ${(attitude.yawspeed * 180 / pi).toStringAsFixed(2)}°/s');
        }
          ...
          _receivedData.add('  ├─ [Position] Lat: ${(pos.lat / 1e7).toStringAsFixed(7)}°, ' +
              'Lon: ${(pos.lon / 1e7).toStringAsFixed(7)}°');
          _receivedData.add('  ├─ Alt (MSL): ${_altMSL.toStringAsFixed(2)}m, ' +
              'Alt (Rel): ${_altRelative.toStringAsFixed(2)}m');
          _receivedData.add('  └─ Speed: N: ${vx.toStringAsFixed(1)}m/s, ' +
              'E: ${vy.toStringAsFixed(1)}m/s, ' +
              'D: ${(pos.vz / 100).toStringAsFixed(1)}m/s, ' +
          ...
          _groundSpeed = hud.groundspeed; // Cập nhật lại ground speed từ VfrHud vì chính xác hơn
          needUIUpdate = true;
          
          _receivedData.add('  ├─ [VFR HUD] Airspeed: ${_airSpeed.toStringAsFixed(1)}m/s, ' +
              'Groundspeed: ${_groundSpeed.toStringAsFixed(1)}m/s');
          _receivedData.add('  └─ Alt: ${hud.alt.toStringAsFixed(1)}m, ' +
              'Climb: ${hud.climb.toStringAsFixed(1)}m/s, ' +
              'Heading: ${hud.heading}°, ' +
              'Throttle: ${hud.throttle}%');
        }
        else if (frm.message is RcChannelsRaw) {
          var rc = frm.message as RcChannelsRaw;
          _receivedData.add('  ├─ [RC Channels] Chan1: ${rc.chan1Raw}, Chan2: ${rc.chan2Raw}, ' +
              'Chan3: ${rc.chan3Raw}, Chan4: ${rc.chan4Raw}');
          _receivedData.add('  └─ Chan5: ${rc.chan5Raw}, Chan6: ${rc.chan6Raw}, ' +
              'Chan7: ${rc.chan7Raw}, Chan8: ${rc.chan8Raw}, RSSI: ${rc.rssi}');
        }
          ...
          _receivedData.add('  ├─ [GPS] Fix: ${_gpsFixType}, ' +
              'Satellites: ${_satellites}');
          _receivedData.add('  └─ Velocity: ${gps.vel / 100}m/s, ' +
              'Course: ${gps.cog / 100}°');
        }
          ...
          try {
            _receivedData.add('  └─ [StatusText] [${_getStatusSeverity(text.severity)}] ${text.text}');
          } catch (e) {
          ...
          _receivedData.add('  └─ [CommandAck] Command: ${ack.command}, Result: ${_getCommandResult(ack.result)}');
        }
          ...
          _receivedData.add('  ├─ [Param] ID: $paramId (${param.paramIndex}/${param.paramCount})');
          _receivedData.add('  ├─ Value: ${param.paramValue}, Type: ${_getParamType(param.paramType)}');
          _receivedData.add('  └─ Raw bytes: ${param.paramId.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join(' ')}');
          ...
          if (param.paramIndex == param.paramCount - 1) {
            ...
            _receivedData.add("✓ Đã nhận xong tất cả parameters (${_parameters.length} parameters)");
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                showDialog(
                  ...
                  builder: (context) => AlertDialog(
                    ...
                    content: const Text('Bạn có muốn xem danh sách parameters không?'),
                    actions: [
                      TextButton(
          ...
          _receivedData.add('  ├─ [IMU] Accel: X: ${imu.xacc / 1000}g, Y: ${imu.yacc / 1000}g, Z: ${imu.zacc / 1000}g');
          _receivedData.add('  └─ Gyro: X: ${imu.xgyro / 1000}rad/s, Y: ${imu.ygyro / 1000}rad/s, Z: ${imu.zgyro / 1000}rad/s');
        }
        else if (frm.message is BatteryStatus) {
          var bat = frm.message as BatteryStatus;
          _receivedData.add('  ├─ [Battery] ID: ${bat.id}, ' + 
              'Voltage: ${bat.voltages[0] != 0 ? (bat.voltages[0] / 1000).toStringAsFixed(2) : "N/A"}V');
          _receivedData.add('  └─ Current: ${bat.currentBattery / 100}A, ' +
              'Level: ${bat.batteryRemaining}%, ' + 
              'Function: ${_getBatteryFunction(bat.batteryFunction)}');
        }
        else if (frm.message is RawImu) {
          var imu = frm.message as RawImu;
          _receivedData.add('  ├─ [Raw IMU] Accel: X: ${imu.xacc}, Y: ${imu.yacc}, Z: ${imu.zacc}');
          _receivedData.add('  └─ Gyro: X: ${imu.xgyro}, Y: ${imu.ygyro}, Z: ${imu.zgyro}, Mag: X: ${imu.xmag}, Y: ${imu.ymag}, Z: ${imu.zmag}');
        }
  ...
  void _updateAvailablePorts() {
    try {
      ...
      if (_availablePorts.isNotEmpty && !_availablePorts.contains(_selectedPort)) {
        setState(() {
  ...
  void _connectToPort() {
    ...
    try {
      ...
      if (_serialPort!.openReadWrite()) {
        ...
        setState(() {
          _isConnected = true;
          _receivedData.add("Đã kết nối tới $_selectedPort với tốc độ $_baudRate baud");
        });
    ...
    if (_serialPort != null && _serialPort!.isOpen) {
      try {
        ...
        if (data.isNotEmpty) {
          ...
          final String timeStr = DateTime.now().toIso8601String().substring(11, 23);
          final String hexData = data.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join(' ');
          
          setState(() {
            _receivedData.add("[$timeStr] Raw data [${data.length} bytes]");
            _receivedData.add("  └─ HEX: ${hexData.length > 10000 ? hexData.substring(0, 10000) + '...' : hexData}");
          });
  ...
  void _addFlightSummary() {
    ...
    _receivedData.add('[$timeStr] ┌── Flight Controller Status ──────────────────────');
    _receivedData.add('        ├── Mode: $_currentMode | ${_isArmed ? "ARMED" : "DISARMED"}');
    if (_currentWaypoint >= 0 && _totalWaypoints > 0) {
      _receivedData.add('        ├── Waypoint: $_currentWaypoint / $_totalWaypoints');
    }
    if (_homePosition.isNotEmpty) {
      _receivedData.add('        ├── Home: Lat: ${_homePosition['lat']?.toStringAsFixed(7)}, Lon: ${_homePosition['lon']?.toStringAsFixed(7)}, Alt: ${_homePosition['alt']?.toStringAsFixed(2)}m');
    }
    if (_ekfStatus != 'Unknown') {
      _receivedData.add('        ├── EKF: $_ekfStatus');
    }
    _receivedData.add('        ├── Attitude: Roll: ${_roll.toStringAsFixed(2)}°, Pitch: ${_pitch.toStringAsFixed(2)}°, Yaw: ${_yaw.toStringAsFixed(2)}°');
    _receivedData.add('        ├── Altitude: MSL: ${_altMSL.toStringAsFixed(2)}m, Relative: ${_altRelative.toStringAsFixed(2)}m');
    _receivedData.add('        ├── Speed: Ground: ${_groundSpeed.toStringAsFixed(1)}m/s, Air: ${_airSpeed.toStringAsFixed(1)}m/s');
    _receivedData.add('        ├── GPS: ${_gpsFixType}, Satellites: $_satellites');
  ...
  void _requestAllParameters() {
    ...
    setState(() {
      ...
      _receivedData.add("Đang yêu cầu tất cả các parameter từ flight controller...");
    });
    ...
    try {
      ...
      setState(() {
        _receivedData.add("Đã gửi yêu cầu danh sách parameter (${bytesWritten} bytes)");
  ...
  void _requestParameter(String paramName) {
    ...
    setState(() {
      _receivedData.add("Đang yêu cầu parameter '$paramName' từ flight controller...");
    });
    ...
    try {
      ...
      setState(() {
        _receivedData.add("Đã gửi yêu cầu parameter '${paramName}' (${bytesWritten} bytes)");
        _receivedData.add("  └─ HEX: $hexData");
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('MAVLink UART Monitor - ${_isConnected ? "Đã kết nối $_selectedPort" : "Chưa kết nối"}'),
        actions: [
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (line.contains('[Heartbeat]')) {
            ...
            if (line.contains('ERROR') || line.contains('CRITICAL') || line.contains('ALERT') || line.contains('EMERGENCY')) {
          ...
          return Text(
            ...
            style: TextStyle(
              ...
              fontFamily: 'Courier',
              fontSize: 12,
              fontWeight: line.contains('[StatusText]') && 
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_homePosition.isNotEmpty)
                        _buildValueDisplay(
                          label: 'HOME',
                          value: 'Lat: ${_homePosition['lat']?.toStringAsFixed(7)}, Lon: ${_homePosition['lon']?.toStringAsFixed(7)}, Alt: ${_homePosition['alt']?.toStringAsFixed(2)}m',
                          color: Colors.blue,
                          isImportant: false,
                        ),
                      if (_ekfStatus != 'Unknown')
                        ...
                        child: Column(
                          children: [
                            ...
                            _buildValueDisplay(
                              label: 'FIX',
                              value: _gpsFixType,
                              color: _gpsFixType == 'No Fix' ? Colors.red : 
                                     _gpsFixType == '2D Fix' ? Colors.orange : 
                                     Colors.green,
                              isImportant: true,
                            ),
                            _buildValueDisplay(
                              label: 'SATELLITES',
                              value: '$_satellites',
                              color: _satellites < 5 ? Colors.red : 
                                     _satellites < 8 ? Colors.orange : 
                                     Colors.green,
                              isImportant: false,
                            ),
                          ],
                        ...
                        child: Column(
                          children: [
                            ...
                            _buildValueDisplay(
                              ...
                              color: _batteryPercent < 20 ? Colors.red : 
                                     _batteryPercent < 50 ? Colors.orange : 
                                     Colors.green,
                              isImportant: true,
                            ),
                          ],
  ...
  Widget _buildSpeedIndicator() {
    return Container(
      ...
      child: CustomPaint(
        painter: SpeedIndicatorPainter(speed: _groundSpeed),
      ),
    );
  }
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(
                  ...
                  topRight: _batteryPercent == 100 ? const Radius.circular(3) : Radius.zero,
                  bottomRight: _batteryPercent == 100 ? const Radius.circular(3) : Radius.zero,
                ),
              ),
            ),
          ),
  ...
  void _requestAllDataStreams() {
    ...
    try {
      ...
      setState(() {
        _receivedData.add("Đã gửi yêu cầu dữ liệu telemetry tới flight controller");
      });
    } catch (e) {
      setState(() {
  ...
  void _requestDataStream(int streamId, int rate) {
    ...
    try {
      ...
      _sequence = (_sequence + 1) % 255; // Tăng sequence và giữ trong khoảng 0-255
      ...
      print("Đã gửi yêu cầu dữ liệu Stream ID: $streamId với tần suất: $rate Hz");
    } catch (e) {
      setState(() {
...
class _ParameterListScreenState extends State<ParameterListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Danh sách Parameters (${filteredEntries.length}/${widget.parameters.length})'),
        actions: [
          IconButton(
            icon: const Icon(Icons.save),
            onPressed: () {
              // Tạo text để copy vào clipboard
              final paramText = sortedEntries.map((e) => '${e.key}=${e.value}').join('\n');
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Chức năng lưu sẽ được thêm sau!'))
              );
            },
                            ...
                            builder: (context) => Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 8),
                                  Text('Giá trị: ${entry.value}', style: const TextStyle(fontSize: 16)),
                                  const SizedBox(height: 16),
                                  Row(
...
class AttitudeIndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = -30; i <= 30; i += 10) {
      ...
      final markY = -i * radius / 40; // Scale để vạch nằm trong phạm vi của chỉ báo
      ...
      if (i % 20 == 0) {
        ...
        textPainter.layout();
        textPainter.paint(canvas, Offset(width + 5, markY - textPainter.height / 2));
        textPainter.paint(canvas, Offset(-width - 5 - textPainter.width, markY - textPainter.height / 2));
      }
    }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint bgPaint = Paint()
      ..shader = LinearGradient(
        ...
        colors: [Colors.blue[900]!, Colors.blue[600]!, Colors.blue[300]!, Colors.green],
    ...
    for (int i = 0; i <= maxAltitude.toInt(); i += 10) {
      ...
      if (isMajorTick) {
        ...
        textPainter.layout();
        textPainter.paint(canvas, Offset(width * 0.25, y - textPainter.height / 2));
      }
    }
    
    // Vẽ con trỏ hiện tại
    final pointerY = height - (height * min(altitude, maxAltitude) / maxAltitude);
    ...
    textPainter.paint(canvas, Offset(centerX - textPainter.width / 2, pointerY - 20));
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= maxSpeed.toInt(); i += 5) {
      ...
      if (isMajorTick) {
        ...
        final textY = centerY + textRadius * sin(angle) - textPainter.height / 2;
    ...
    textPainter.layout();
    textPainter.paint(canvas, Offset(centerX - textPainter.width / 2, centerY + 30));
  }
